...
export async function POST(req:Request){
  ...
  const ev=await admin.from('signature_events').insert({law_firm_id:r.law_firm_id,request_id:r.id,signer_id:s.id,event_type:'evidencia_facial_recebida',ip:String(req.headers.get('x-forwarded-for')||req.headers.get('x-real-ip')||'').split(',')[0].trim(), user_agent:String(req.headers.get('user-agent')||''), mime:selfie.type, metadata:{selfie_path:selfiePath,cpf:cpf||s.cpf||null}});
