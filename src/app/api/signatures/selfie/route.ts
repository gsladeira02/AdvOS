...
export async function POST(req:Request){
  ...
  const ev=await admin.from('signature_events').insert({
    law_firm_id:r.law_firm_id,
    request_id:r.id,
    signer_id:s.id,
    event_type:'evidencia_facial_recebida',
    ip,
    ...
    metadata:{
      ...
      cpf:cpf||s.cpf||null,
      mime_type:selfie.type||'image/jpeg'
    }
  });
