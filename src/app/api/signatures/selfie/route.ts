...
export async function POST(req:Request){
  const f=await req.formData(); const token=String(f.get('token')||'').trim(); const selfie=f.get('selfie'); const docPhoto=f.get('document_photo');
  if(!(selfie instanceof File)) return NextResponse.json({ok:false,error:'Selfie não enviada.'},{status:400});
  const admin=createAdminSupabase();
  const {data:r}=await admin.from('signature_requests').select('id,law_firm_id,require_selfie,require_document_photo,expires_at').eq('public_token',token).maybeSingle();
  ...
  const {data:s}=await admin.from('signature_signers').select('id').eq('request_id',r.id).order('created_at').limit(1).maybeSingle();
  if(!s) return NextResponse.json({ok:false,error:'Signatário não encontrado.'},{status:404});
  const bucket='signature-evidence'; const ext='jpg'; const selfiePath=`${r.law_firm_id}/${r.id}/selfie-${crypto.randomUUID()}.${ext}`;
