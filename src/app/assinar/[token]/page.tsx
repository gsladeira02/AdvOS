...
export default async function PublicSignPage({params}:{params:Promise<{token:string}>}){
  ...
  if(!r){
    const {data:s}=await admin.from('signature_signers').select('id,request_id,name,phone,email,status,role,signer_token,signer_order').eq('signer_token',token).maybeSingle();
